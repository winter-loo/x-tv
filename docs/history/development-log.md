...
> 原 README 的开发过程快照；当前使用方式见根目录 README，发布状态见 ../release.md。
...
- 确认：从时间线直接打开帖子详情；在详情正文上再次确认进入全屏阅读。全屏返回详情，详情返回原时间线帖子。
...
回归与投影仪复测见 [UX 验收记录](../validation/ux-startup-navigation-media.md)。真机复测脚本只浏览和控制媒体，不点赞或发布：
...
验证记录见 [评论详情验收](../validation/comment-details.md)。
...
回归测试 `tests/external-links.spec.mjs`，真机验收 `node scripts/check-external-links.mjs`（只读，不点赞不评论）。验证记录见 [外链阅读验收](../validation/issue-20-external-links.md)。
...
回归测试 `app/src/test/.../HandoffTest.java` 与 `tests/external-links.spec.mjs`，真机验收 `node scripts/check-reader-foreground.mjs`（只读）。验证记录见 [启动与返回界面验收](../validation/issue-22-reader-foreground.md)。
...
回归测试见 `tests/like.spec.mjs`。真机验收 `node scripts/check-like-feedback.mjs` 在按键前替换为合成桥接，不向 X 发出写入，记录已安装 APK 哈希、DOM/动画帧时序和未完成请求下的导航。详见 [喜欢即时反馈验收](../validation/issue-13-optimistic-like.md)。

外链返回后会在后台恢复 X 页面，真实写入准备等待元数据就绪，不占用阅读前台。回归命令 `node scripts/check-write-recovery.mjs` 只检查真实准备链路，不提交点赞。见 [外链返回后的写入恢复验收](../validation/issue-13-write-recovery.md)。
...
回归测试 `tests/refresh.spec.mjs`，真机验收 `node scripts/check-timeline-refresh.mjs`（只读）。验证记录见 [顶部刷新验收](../validation/issue-21-timeline-refresh.md)。
...
回归测试 `tests/layout.spec.mjs`，真机验收 `node scripts/check-reader-layout.mjs`（只读，`--shots` 可另存 1920×1080 截图）。验证记录见 [阅读器布局验收](../validation/issue-14-reader-layout.md)。
...
回归测试 `tests/time-stats.spec.mjs`，真机验收 `node scripts/check-time-stats.mjs`（只读）。验证记录见 [时间与统计验收](../validation/issue-16-time-stats.md)。
...
回归测试 `tests/reading-scroll.spec.mjs`，真机验收 `node scripts/check-reading-flow.mjs`（只读）。验证记录见 [长正文阅读验收](../validation/issue-19-23-reading.md)。
...
回归测试 `tests/media.spec.mjs`，真机验收 `node scripts/check-media.mjs`（只读）。验证记录见 [媒体验收](../validation/issue-15-17-18-media.md)。

## 当前进度（2026-09-07）

#1 平衡阅读布局已完成验收：9 项 Firefox DOM 回归、150 条真实帖子连续导航、真实 X Article 显示、详情往返、返回顶部、Google 登录与冷启动登录保持均通过。GeckoView 已更新至 `155.0.20260903215306`，修复真机 RGB PNG 解码崩溃。详见 [验收记录](../validation/issue-1.md)。
...
测试边界和真机 PNG 解码复现见 [tests/README.md](../../tests/README.md)，#1 的验收证据见 [docs/validation/issue-1.md](../validation/issue-1.md)。
