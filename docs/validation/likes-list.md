...
**右键仍然是媒体**。工单选项写的是「顶部左右键切换」，实现时只用了左键：右键在首帖上原本就是进入媒体面板，抢走它会让首帖（最常读的一条）失去媒体入口。两个列表用一个切换键已经完备，`tests/likes.spec.mjs` 里「the media pane keeps left, and only the top post switches lists」专门钉住了这一点。
