...
public class TvKeyRouter {
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_UP:
                if (mBridge == null || !mBridge.isConnected()) {
                    Log.w(TAG, "Bridge not connected, passing DPAD_UP to GeckoView");
                    return false;
                }
            ...
            case KeyEvent.KEYCODE_DPAD_DOWN:
                if (mBridge == null || !mBridge.isConnected()) {
                    Log.w(TAG, "Bridge not connected, passing DPAD_DOWN to GeckoView");
                    return false;
                }
                if (now - mLastMoveTime >= MOVE_THROTTLE_MS) {
                    mLastMoveTime = now;
                    Log.e(TAG, "Routing DPAD_DOWN to bridge.sendMove('down')");
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
                if (mBridge == null || !mBridge.isConnected()) {
                    Log.w(TAG, "Bridge not connected, passing DPAD_LEFT to GeckoView");
                    return false;
                }
                if (now - mLastMoveTime >= MOVE_THROTTLE_MS) {
                    mLastMoveTime = now;
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                if (mBridge == null || !mBridge.isConnected()) {
                    Log.w(TAG, "Bridge not connected, passing DPAD_RIGHT to GeckoView");
                    return false;
                }
                if (now - mLastMoveTime >= MOVE_THROTTLE_MS) {
                    mLastMoveTime = now;
            ...
            case KeyEvent.KEYCODE_ENTER:
                if (mBridge == null || !mBridge.isConnected()) {
                    Log.w(TAG, "Bridge not connected, passing ENTER to GeckoView");
                    return false;
                }
    ...
    private void handleBackKey() {
        ...
        mBridge.sendBack(handledByWeb -> {
            mMainHandler.post(() -> {
                Log.i(TAG, "Back handled by web extension: " + handledByWeb);
                if (!handledByWeb) {
                    performFallbackBack();
                }
            });
    ...
    private void performFallbackBack() {
        if (mCanGoBack && mSession != null) {
            Log.i(TAG, "Session going back in history...");
            mSession.goBack();
            return;
        }
        if (mExitHandler != null) {
            Log.i(TAG, "No history to go back to, exiting Activity.");
