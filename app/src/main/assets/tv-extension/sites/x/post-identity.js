...
window.TvXPostIdentity = window.TvXPostIdentity || (function() {
    function ownStatusLink(article, findTimelineLink) {
        function isOwnTimestamp(link) {
            ...
            if (url.origin !== location.origin || !/^\/[^/]+\/status\/\d+$/.test(url.pathname)) return false;
        ...
        const paths = new Set(candidates.map(link => new URL(link.getAttribute('href'), location.href).pathname));
        return paths.size === 1 ? candidates[0] : null;
    }

    return { statusLink: ownStatusLink };
})();
