...
const source = await readFile(new URL('../app/src/main/assets/tv-extension/sites/x/write-api.js',import.meta.url),'utf8');
function fixture({rotate=false}={}) {
    let listener, cookie='ct0=session-csrf', transactions=[];
    const definitions={};
    for(const name of ['FavoriteTweet','UnfavoriteTweet','CreateTweet','TweetResultByRestId']) {
...
test('metadata uses current definitions, current flags and distinct method/path signatures without sending a mutation',async()=>{
    const f=fixture();const result=await f.send({command:'writePrepare',operation:'CreateTweet'});
...
test('metadata rejects unsupported operations and a session rotating during preparation',async()=>{
    const f=fixture();expect(await f.send({command:'writePrepare',operation:'DeleteTweet'})).toEqual({error:'invalid'});
    expect(f.transactions).toHaveLength(0);
    expect(f.send({command:'activate'})).toBeUndefined();
    const rotating=fixture({rotate:true});expect((await rotating.send({command:'writePrepare',operation:'CreateTweet'})).error).toBe('not_ready');
});
